...
class PAPLParser:
    ...
    def _extract_tables(self, doc: Document) -> List[Dict[str, Any]]:
        """Extract tables with full data and row numbering"""
        tables = []
        for i, table in enumerate(doc.tables):
            rows = []
            rows_with_numbers = []  # NEW: Store rows with their row numbers
            ...
            if rows:
                tables.append({
                    'index': i,
                    'table_number': i + 1,  # NEW: 1-indexed table number for display
                    'rows': len(rows),
                    'cols': len(rows[0]) if rows else 0,
                    'data': rows,
                    'data_with_row_numbers': rows_with_numbers,  # NEW: Detailed row data
        ...
        for table in tables:
            ...
            if is_pricing:
                ...
                for row_idx, row in enumerate(table['body']):
                    if len(row) >= 2:  # At least item name and price
                        pricing_items.append({
                            ...
                            'location': {  # NEW: Precise location tracking
                                'table_number': table['table_number'],
                                'table_index': table['index'],
                                'row_number': row_idx + 1,  # Body rows start at 1 (header is 0)
                                'absolute_row': row_idx + 2  # Accounting for header row in table
                            },
                            'metadata': {
                                'row_length': len(row),
                                'all_cells': row
                ...
                pricing_tables.append({
                    'table_index': table['index'],
                    'table_number': table['table_number'],  # NEW: Add table number
                    'header': table['header'],
        ...
        for para in paragraphs:
            ...
            if has_rule_keyword and len(para['text']) > 50:  # Substantial rule text
                ...
                rule = {
                    'text': para['text'],
                    'location': {  # NEW: Precise location tracking
                        'paragraph_number': para['index'] + 1,  # 1-indexed for display
        ...
        for para in paragraphs:
            ...
            if para['is_heading']:
                ...
                current_section = {
                    'heading': para['text'],
                    'level': para['level'],
                    'location': {  # NEW: Location of heading
                        'paragraph_number': para['index'] + 1,
                        'paragraph_index': para['index']
                    },
                    'paragraphs': []
                }
                ...
                current_section['paragraphs'].append({
                    'text': para['text'],
                    'style': para['style'],
                    'location': {  # NEW: Location of each paragraph
                        'paragraph_number': para['index'] + 1,
                        'paragraph_index': para['index']
                    }
                })
